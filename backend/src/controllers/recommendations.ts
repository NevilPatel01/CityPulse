...
export const saveRecommendation = async (req: Request, res: Response) => {
    try {
        ...
        // Get updated count
        const countResult = await query(
            'SELECT COUNT(*)::integer as saves_count FROM recommendation_saves WHERE recommendation_id = $1',
            [id]
        );
    ...
    try {
        ...
        const result = await query(`
            SELECT 
                ...
                r.views_count,
                r.likes_count,
                (SELECT COUNT(*) FROM recommendation_saves WHERE recommendation_id = r.id),
                r.created_at,
                u.username,
                u.full_name,
                rc.name as category_name,
